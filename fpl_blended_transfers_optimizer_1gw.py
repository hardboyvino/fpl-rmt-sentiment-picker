...
# Weights: each = points_actual_mean / (section max points_actual_mean)
POS_WEIGHTS_1GW = {
    "Defender": 0.8545,
    "Forward": 1.0000,   # base (highest Points_actual mean: 3.85)
    "Goalkeeper": 0.7351,
    "Midfielder": 0.8857,
}
...
PRICE_WEIGHTS_1GW = {
    ...
    8.5: 0.3478,
    9.0: 0.2826,
    14.0: 1.0000,  # base (highest Points_actual mean: 11.50)
    14.5: 0.3913,
}

TEAM_WEIGHTS_1GW = {
    "Arsenal": 1.0000,        # base (highest Points_actual mean: 4.65)
    "Newcastle": 0.9634,
    "Man City": 0.9495,
    "Bournemouth": 0.9279,
    "Sunderland": 0.8473,
    "Everton": 0.8194,
    "Crystal Palace": 0.8130,
    "Spurs": 0.7613,
    "Liverpool": 0.7581,
    "Fulham": 0.7398,
    "Aston Villa": 0.6946,
    ...
    "Brentford": 0.5505,
    "West Ham": 0.4882,
    "Nott'm Forest": 0.4602,
    "Wolves": 0.3978,
}

# Effectiveness: (points_actual_mean / points_score_mean),
# then min-max normalized within each section to the range [0.1, 1.0]
POS_EFFECTIVENESS_1GW = {
    "Defender": 1.0000,
    "Forward": 0.6507,
    "Goalkeeper": 0.1000,
    "Midfielder": 0.4712,
...
PRICE_EFFECTIVENESS_1GW = {
    ...
    7.0: 0.2297,
    7.5: 0.2555,
    8.0: 0.1000,   # section minimum
    8.5: 0.2546,
    9.0: 0.1895,
    14.0: 1.0000,  # section maximum
    14.5: 0.2496,
}

TEAM_EFFECTIVENESS_1GW = {
    "Arsenal": 0.8137,
    "Newcastle": 1.0000,        # section maximum
    "Man City": 0.9763,
    "Bournemouth": 0.7504,
    "Sunderland": 0.5271,
    "Everton": 0.6773,
    "Crystal Palace": 0.7856,
    "Spurs": 0.3058,
    "Liverpool": 0.6246,
    "Fulham": 0.4655,
    "Aston Villa": 0.5875,
    ...
    "Brentford": 0.4060,
    "West Ham": 0.4755,
    "Nott'm Forest": 0.1693,
    "Wolves": 0.1000,           # section minimum
}
...
def adjust_points(row: pd.Series) -> float:
    ...
    pos_weight = POS_WEIGHTS_1GW.get(row['Position'], 1.0)

    # Position adjustment with updated effectiveness
    pos_effectiveness = POS_EFFECTIVENESS_1GW.get(row['Position'], 1.0)

    # Price bracket adjustment with new effectiveness ratios
    price_bracket = round(float(row['Price']) * 2) / 2
    price_effectiveness = PRICE_EFFECTIVENESS_1GW.get(price_bracket, 1.0)
    price_weight = PRICE_WEIGHTS_1GW.get(price_bracket, 0.5)

    # Team performance adjustment with updated weights
    team_weight = TEAM_WEIGHTS_1GW.get(row['Team'], 0.5)
    team_effectiveness = TEAM_EFFECTIVENESS_1GW.get(row['Team'], 1.0)
...
def enhance_predictions(df: pd.DataFrame) -> pd.DataFrame:
    """
    Enhance the predictions dataframe with adjusted points based on comprehensive analysis
    """
    df = df.copy()
    df['Points_Original'] = df['Points']
...
def map_current_ids_to_csv_rows(
    ...
    for pid in missing_ids:
        ...
        row = dict(Name=nm, Team=tm, Position=pos, Price=price, Points=0.000001, key=_key(nm, tm)) 
...
# Modify the main optimization function to use enhanced predictions
def optimize_k_enhanced(
    k: int,
    pool_df: pd.DataFrame,
    owned_df: pd.DataFrame,
    bank_m: float,
    ...
    """
    Enhanced optimization function incorporating analysis insights
    """
    # Enhance predictions with our analysis-based adjustments
    pool_df = enhance_predictions(pool_df)
    
    # Continue with original optimization logic but with enhanced predictions
    return optimize_k(
        k=k,
        pool_df=pool_df,
        owned_df=owned_df,
        ...
        include_start_names=include_start_names,
        exclude_names=exclude_names,
        include_teams=include_teams,
        exclude_teams=exclude_teams,
        block_add_names=block_add_names,
        solver_msg=solver_msg
    )


def write_enhanced_report(entry_id: int, bank_m: float, owned_df: pd.DataFrame, k_results: Dict[int, dict], outfile: str):
    """
    Enhanced report writing with additional analysis insights
    """
    def row_str(r) -> str:
        return f"{r['Name']:<22} {r['Team']:<15} {r['Position']:<12} {float(r['Price']):>5.1f}  {float(r['Points']):>6.2f} (orig: {float(r['Points_Original']):>6.2f})"

    lines = []
    lines.append(f"Enhanced FPL Optimization for Entry {entry_id}")
    ...
    for k in sorted(k_results.keys()):
        ...
        for team, count in team_counts.items():
            team_weight = TEAM_WEIGHTS_1GW.get(team, 0)
        ...
        for bracket, count in price_brackets.items():
            effectiveness = PRICE_EFFECTIVENESS_1GW.get(bracket, 0)
            lines.append(f"  £{bracket}m: {count} players (Effectiveness: {effectiveness:.2f})")

        # Continue with standard reporting...
        lines.append(f"\nProjected XI points: {res['starting_points']:.2f} | Bench points: {res['bench_points']:.2f}")
        ...
        # Enhanced captain suggestion considering team and position weights
        def captain_score(pid):
            r = df.loc[pid]
            base_score = float(r['Points'])
            team_mult = TEAM_WEIGHTS_1GW.get(r['Team'], 0.5)
            pos_mult = POS_WEIGHTS_1GW.get(r['Position'], 1.0)
...
# Update main() to use enhanced functions
def main():
    args = parse_args()
    ...
    # Load FPL API data and determine bank (same as original)
    ...
    if args.bank is not None:
        ...
        sys.exit("Bank balance not available from API. Please rerun with --bank <amount_in_millions>")
    ...
    outfile = args.outfile or f"enhanced_transfer_suggestions_{args.entry}.txt"
    write_enhanced_report(args.entry, bank_m, owned_df, results, outfile)
